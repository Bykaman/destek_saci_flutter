...
class GProfilSayfasi extends StatefulWidget {
  const GProfilSayfasi({super.key});
  @override
  State<GProfilSayfasi> createState() => _GProfilSayfasiState();
}
...
class _GProfilSayfasiState extends State<GProfilSayfasi> {
  ...
  // Stil/Renk (diğer sayfalarla aynı)
  final Color _pageBg = const Color(0xFFF3F4F6);
  final Color _cardFill = const Color(0xFFEFF6FF);
  ...
  String _fmtCurr(num x) => '${NumberFormat('#,##0.00', 'tr_TR').format(x)} ₺';

  // Odaktayken boşsa örnek gösteren alan
  Widget _numField({
  ...
  void initState() {
    super.initState();
    for (final f in [_fEt, _fK1, _fK2, _fK3, _fK4, _fFiyat]) {
      f.addListener(() => setState(() {}));
    }
    _hesapla(); // başlangıçta 0 değerler
  }

  @override
  void dispose() {
    _et.dispose(); _k1.dispose(); _k2.dispose(); _k3.dispose(); _k4.dispose();
    _fiyatKg.dispose(); _boyMm.dispose(); _adet.dispose();
    _fEt.dispose(); _fK1.dispose(); _fK2.dispose(); _fK3.dispose();
    _fK4.dispose(); _fFiyat.dispose();
    super.dispose();
  }

  void _hesapla() {
    final et = _p(_et.text);
    final k1 = _p(_k1.text);
    final k2 = _p(_k2.text);
    final k3 = _p(_k3.text);
    final k4 = _p(_k4.text);
    final fiyatKg = _p(_fiyatKg.text);

    final toplam = k1 + k2 + k3 + k4; // mm
    final bukulmePayi = et * 6;       // 3 köşe × 2 pay
    netGenislik = (toplam - bukulmePayi).clamp(0, double.infinity);
  ...
  void _hesaplaDetay() {
    if (!_detay) {
      _toplamMetre = 0; _toplamKilo = 0; _toplamTl = 0;
      setState(() {}); return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('G PROFİL Hesaplama'),
        centerTitle: true,
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            _numField(c: _k1,   f: _fK1,   label: '1. Kenar (mm)',     example: 'Örn: 25'),
            _numField(c: _k2,   f: _fK2,   label: '2. Kenar (mm)',     example: 'Örn: 30'),
            _numField(c: _k3,   f: _fK3,   label: '3. Kenar (mm)',     example: 'Örn: 25'),
            _numField(c: _k4,   f: _fK4,   label: '4. Kenar (mm)',     example: 'Örn: 10'),

...
class UProfilSayfasi extends StatefulWidget {
  const UProfilSayfasi({super.key});
  @override
  State<UProfilSayfasi> createState() => _UProfilSayfasiState();
}
...
class _UProfilSayfasiState extends State<UProfilSayfasi> {
  ...
  // Odaklar: sadece odaktayken “Örn:” ipucu göstermek için
  ...
  // Odakta ve boşsa “Örn:” ipucu gösteren, diğer tüm stili KORUYAN field
  Widget _numField({
    required TextEditingController c,
    required FocusNode f,
    required String label,
    required String example, // "Örn: 23" vb.
  }) {
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          labelText: label,
          hintText: showHint ? example : null, // sadece odakta ve boşken
          filled: true,
        ...
        onSubmitted: (_) => _hesapla(), // Mevcut davranış aynı
      ),
    );
  }
  ...
  void initState() {
    super.initState();
    // Odak değişiminde yeniden çiz (ipucu için)
    for (final f in [_fEt, _fK1, _fTaban, _fK2, _fFiyat]) {
  ...
  void _hesapla() {
    ...
    final toplam = k1 + tb + k2;    // mm
    final kosePayi = et * 4;        // 2 köşe × 2 pay
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('U PROFİL Hesaplama'),
        centerTitle: true,
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            // SADECE BURADA hint davranışı ekledik; stil/renk/hesap aynı
            _numField(c: _et,    f: _fEt,    label: 'Et Kalınlığı (mm)',  example: 'Örn: 1,20'),
            _numField(c: _k1,    f: _fK1,    label: '1. Kenar (mm)',      example: 'Örn: 23'),
            _numField(c: _taban, f: _fTaban, label: 'Taban (mm)',         example: 'Örn: 23'),
            _numField(c: _k2,    f: _fK2,    label: '2. Kenar (mm)',      example: 'Örn: 23'),
